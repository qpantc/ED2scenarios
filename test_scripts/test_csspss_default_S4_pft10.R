...
csspss_path <- file.path(csspss_config_path,'test','test_10')
pft_xml_path <- file.path(csspss_config_path,'test','test_10')
...
for(i in 1:1){#nrow(simulating_df)
  ...
  ed2in_scenar$INCLUDE_THESE_PFT = c(10) #sort(unique(css_pss_df$pft[which(css_pss_df$SITE_ID==site_id )]))
