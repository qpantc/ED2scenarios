...
csspss_path <- '/Volumes/Sd1TB/MODEL_DRIVER/ED2/ICOS/csspss/site' # 结尾不带/
...
run_type_list <- c('S4','S5') #'S0','S1','S2','S3','S4','S5','S6'
list_dir <- list()
for(run_type in run_type_list){
  ...
  for(simulation_i in simulations_ids){
    ...
    if (run_type == 'S0') {
      ...
      ed2in_scenar <- read_ed2in(ED2IN_ref)
      ed2in_scenar$SFILIN = sub("\\.css$", "", file.path(csspss_path, paste0(site_id,'.css')))
    } 
